...
public class Mode : MonoBehaviour
{
  ...
  void Update(){
    ...
    if(machineController.GetComponent<ControlllerStep>().action == 1){
      ...
      if(step==1 && state[step-1]==1)      {step++; Action= 0;}
      else if(step==2 && state[step-1]==1) {step++; Action= 0;}
      else if(step==3 && state[step-1]==0) {step++; Action= 0;}
      else if(step==4 && state[step-1]==1) {step++; Action= 0;}
      else if(step==5 && state[step-1]==0) {step++; Action= 0;}
      else if(step==6 && state[step-1]==0) {step++; Action= 0;}
      else if(step==7 && state[step-1]==1) {step++; Action= 0;}
      else if(step==8 && state[step-1]==0) {step++; Action= 0;}
      else if(step==9)                     {step++; Action= 0;}
      else{
        ...
        else {time = 0; Environment_light.GetComponent<Light>().color = Color.white; machineController.GetComponent<ControlllerStep>().action = 0;} 
